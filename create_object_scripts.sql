/* 1. создаем таблицу klinz_transit и индексы к ней */
CREATE TABLE KLINZ_TRANSIT AS SELECT * FROM KLINZ
where 1=2;
 
TRUNCATE TABLE klinz;

CREATE INDEX KLINZ_TR_SK ON KLINZ_TRANSIT
(SK_1)
LOGGING
TABLESPACE USERS
PCTFREE    5
...
CREATE UNIQUE INDEX KLINZ_TR_PK ON KLINZ_TRANSIT
(SK_1, GL_1, ZB2_1)
LOGGING
TABLESPACE USERS
PCTFREE    5
...
ALTER TABLE KLINZ_TRANSIT ADD (
  CONSTRAINT KLINZ_TR_PK PRIMARY KEY (SK_1, GL_1, ZB2_1) USING INDEX KLINZ_TR_PK  ENABLE VALIDATE);
  
 /* 2. Создаем таблицы для логирования загрузки */ 
  
EXECUTE DBMS_ERRLOG.CREATE_ERROR_LOG('KLINZ_TRANSIT');
EXECUTE DBMS_ERRLOG.CREATE_ERROR_LOG('CORRDB', null , null , null, TRUE);

 /* 3. создаем нужные типы  */
 --(Kpl,name_TABLE,name_column,value_column,priority,priority_field,name_uSer,computer_name)
CREATE OR REPLACE TYPE t_corrdb_rec IS OBJECT ( 
                                  KPL                VARCHAR2(6 BYTE),
                                  NAME_TABLE         VARCHAR2(20 BYTE),
                                  NAME_COLUMN        VARCHAR2(20 BYTE),
                                  VALUE_COLUMN       VARCHAR2(2000 BYTE),
                                  PRIORITY           NUMBER(2),
                                  PRIORITY_FIELD     VARCHAR2(50 BYTE),
                                  NAME_USER          VARCHAR2(50 BYTE),
                                  COMPUTER_NAME      VARCHAR2(30 BYTE));
                                  
CREATE OR REPLACE TYPE t_corrdb_tab AS TABLE OF t_corrdb_rec;
...
CREATE OR REPLACE PACKAGE BODY KLINZ_LOAD AS
  ...
  BEGIN -- блок инициализации
        ...
        -- режим загрузки (0 -  устаревший, 1 - актуальный) 1 - по умолчанию
        SET_LOAD_REGIM(1);
  END;
    /
...
BEGIN
  SYS.DBMS_SCHEDULER.CREATE_JOB
    (
       job_name        => 'GEOBDTOKLINZ'
      ,start_date      => TO_TIMESTAMP_TZ('2020/12/08 10:13:16.188961 +07:00','yyyy/mm/dd hh24:mi:ss.ff tzr')
      ,repeat_interval => 'FREQ=DAILY;INTERVAL=1;BYHOUR=2;BYMINUTE=0;BYSECOND=0;'
      ,end_date        => NULL
      ,job_class       => 'DEFAULT_JOB_CLASS'
      ,job_type        => 'STORED_PROCEDURE'
      ,job_action      => 'KLINZ_LOAD.LOADER'
      ,comments        => 'Заполнение таблицы KLINZ из geoBD'
    );
